...
int UdpServerProtocol::ProtocolSendRawDataToRemote(uint8_t* data, size_t size, boost::asio::ip::udp::endpoint& remote_) {
  if (remote_.port() != 0) {
    int send_len = socket_->send_to(boost::asio::buffer(data, size), remote_);
    #ifndef NDEBUG
      std::cout << "Sent " << send_len << "Bytes to " << protocol_config_.udp_server_ip_ << ":" << protocol_config_.udp_server_port_ << std::endl;
    #endif
    return send_len;
